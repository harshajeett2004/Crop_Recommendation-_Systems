...
Data Pipeline
Historical Dataset: We used the Kaggle Crop Recommendation Dataset v2 for initial training. This dataset includes parameters such as nitrogen, phosphorus, potassium levels, temperature, humidity, pH, and rainfall.

Real-Time Sensor Data Collection: Sensor readings are collected via a microcontroller and transmitted to the prediction system.

Preprocessing: Real-time data is cleaned and scaled to match the format of the training dataset.

Model Prediction: A Random Forest Classifier trained on historical data predicts the best-suited crop for current conditions.
